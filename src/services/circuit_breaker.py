...
class CircuitBreaker:
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        async with self._lock:
            current_state = self.state  # This triggers automatic transition if needed
            
            # OPEN state: fail fast
            if current_state == CircuitState.OPEN:
                retry_after = self.config.timeout_seconds
                if self._opened_at:
                    retry_after = max(0, self.config.timeout_seconds - (time.time() - self._opened_at))
                raise CircuitBreakerOpenError(retry_after)
            
            # HALF_OPEN state: allow limited calls
            if current_state == CircuitState.HALF_OPEN:
                if self._half_open_calls >= self.config.half_open_max_calls:
                    # Already at max concurrent calls in half-open
    ...
    async def _on_failure(self) -> None:
        ...
        async with self._lock:
            ...
            # In HALF_OPEN state, transition back to OPEN immediately on failure
            if self._state == CircuitState.HALF_OPEN:
    ...
    def _should_open(self) -> bool:
        ...
        failure_count = sum(1 for result in self._recent_results if not result)
        
        # Open if we've reached the failure threshold
        # (even if the window isn't full yet - early protection)
