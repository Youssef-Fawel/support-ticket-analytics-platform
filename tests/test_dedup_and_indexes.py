...
@pytest.mark.asyncio
async def test_strict_idempotency(client: AsyncClient):
    """
    TEST: Multiple ingestions should NOT create duplicate documents.
    """
    # First Run
    resp1 = await client.post("/ingest/run?tenant_id=tenant_a")
    ...
    await client.post("/ingest/run?tenant_id=tenant_a")
    await client.post("/ingest/run?tenant_id=tenant_a")
